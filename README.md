...
11. 分析结果中的单词、搭配和文法都是可点击块，可将原型、文中形式、读音和语境释义存入个人学习数据库。
12. 同一原型在新句子中再次收藏时累计学习次数；遇到新的语境含义时，在原条目下追加独立释义。
13. 提供独立单词库视图；可查看原型、完整读音、释义和遇见次数，并展开查看来源视频与原句。
14. 支持把个人单词库导出为一句一条的 Markdown：`日文（读音）：含义`。
15. 保存提取历史、OCR 检查点和整理检查点；同一视频可以复用已有结果，降低中断后的重复处理和 API 消耗。
...
音频时间。`workspace/learning/library.sqlite3` 保存个人词库的原型、形式、语境释义和出现记录。
